...
clear all
!rm isomip_plus_ocean3_*.nc
InpGrd = 'Ocean3_input_geom_v1.01.nc'
...
icedraft_allTime = ncread(InpGrd,'lowerSurface');
h_allTime = -ncread(InpGrd,'bedrockTopography');
...
for tt = 1:101
    h        = h_allTime(:,:,tt);
    icedraft = icedraft_allTime(:,:,tt);
    GrdNameStr = 'isomip_plus_ocean3_'+string(tt)+'.nc'
    GrdName = char(GrdNameStr)
    xload = ncread(InpGrd,'x');
    yload = ncread(InpGrd,'y');
    [Yload,Xload]=meshgrid(yload,xload);
