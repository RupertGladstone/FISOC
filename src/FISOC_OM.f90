...
CONTAINS
  ...
  SUBROUTINE FISOC_OM_run(FISOC_OM, OM_ImpSt, OM_ExpSt, FISOC_clock, rc)
    ...
    CALL OM_HandleCavity(FISOC_config, FISOC_clock, OM_ImpFB, OM_ExpFB, rc=rc)
    IF (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
         line=__LINE__, file=__FILE__)) &
         CALL ESMF_Finalize(endflag=ESMF_END_ABORT)
  ...
  !------------------------------------------------------------------------------
  SUBROUTINE OM_HandleCavity(FISOC_config, FISOC_clock, OM_ImpFB, OM_ExpFB, rc)
    ...
    REAL(ESMF_KIND_R8)          :: linterpFactor, ISM_dt
