...
More detail of the FISOC specific options is given here.
...
*** move following stuff to later section or GMD paper: ****

also need some discussion on relevant ROMS params in .in file.
DCRIT = 20.0
...
The calculation of pressure at the ice-ocean interface no longer estimates the weight of the ice column based on the assumption of floatation, but rather uses the actual ice thickness to estimate pressure.  The value for ice density defined in iceshelf\_mod.h is used.

***put the pressure equation here***
...
Example FISOC configurations can be found in the repository under FISOC/examples. 

Some of these examples use a ROMS setup which is externally defined (i.e. 
configuration information is not contained in these subdirectories).  
At time of writing the corresponding ROMS setups can be obtained from the 
ROMSIceShelf development repository.

Examples 4 and 5 corresponds to experiments *** in the FISOC model description paper 
...
The corresponding ROMS configuration is called ICESHELF2D
...
This is for running the ISOMIP+ ocn3 and ocn 4 experiments.

\subsubsection{Example 4: Simple floating only test}
A FISOC example using Elmer/Ice and ROMS. 
The Elmer/Ice setup is provided with the example in the subdirectory. 
The corresponding ROMS configuration is called ICESHELF2D\_TOY

\subsubsection{Example 5: Simple grounding line migration test}
A FISOC example using Elmer/Ice and ROMS. 
The Elmer/Ice setup is provided with the example in the subdirectory. 
The corresponding ROMS configuration is called ICESHELF2D\_TOY\_GL
